...
def build_type_instruction(assigned_type: str) -> str:
    ...
    2026-09-12 修复(1)：加入强制覆盖声明。首次实测发现，仅给出类型指令时，
    Claude 会因为学生此前已经完整、正确地回答过某个基础概念问题（例如
    换元三要素），触发 SCL_SYSTEM_PROMPT 里"任务完成推进（含信息重复
    检测）"规则，误判"这个问题已经问过/答过了"，继续沿用旧题目的下一步，
    而不服从这里的换题指令——两条指令打架，教学连续性规则赢了，导致
    assigned_type/actual_type 已经在数据库里正确落地，但学生实际看到的
    仍是旧题。修复方式：显式声明"这是一次全新出题请求，与概念性问题是
    否已经答过无关"，参照"概念切换已确认"那条已验证有效的强制覆盖措辞
    模式。

    2026-09-12 修复(2)：修复(1)上线后连续两次刷新（第3、4次）依然失败,
    actual_type 均为 null（Claude 完全没生成新题）。根因：前端"刷新"
    按钮每次发送的 user 请求文字是同一句固定模板（OPENING_PROMPTS里的
    开场白），到第3次时这句话在对话历史里已经一字不差地出现过好几遍，
    Claude 把"请求文字本身的字面重复"当作比修复(1)指令更强的重复信号，
    直接回复"这个问题你已经回答过三次了，我不会再重复"，转而把学生摁回
    上一题——修复(1)的措辞只覆盖了"概念性问题被答对"这一种重复来源，
    没有覆盖"请求文字本身逐字重复"这一种。补上对应澄清。
    """
    return (
        f"【出题请求：这是一次全新出题请求，请求本身与任何概念性问题是否"
        f"已经被回答过无关】学生点击了「刷新」或刚进入本概念，这是在要求"
        f"你放弃当前正在进行的题目——无论该题目进行到哪一步、无论学生之前"
        f"是否已经完整、正确地回答过与当前题目相关的任何基础概念问题（例如"
        f"换元三要素、u/du是什么、分部积分怎么选u和dv等）。不要因为学生已经"
        f"正确回答过这类概念性问题，就把这次请求理解成'继续推进旧题目的下"
        f"一步'——这次请求的意图是重新出题，不是继续教学。\n"
        f"【重要：请求文字本身的字面重复不代表学生在重复提问】你即将收到"
        f"的这句请求文字，是系统在学生每次点击「刷新」或进入本概念时都会"
        f"原样发送的固定模板，可能与对话历史里更早出现过的请求文字一字不"
        f"差。这种字面重复只是系统实现方式导致的，不代表学生真的在重复"
        f"问同一个问题——请完全忽略这句话是否与历史中出现过的文字相同，"
        f"单纯把它当作'现在请出一道新题'的信号来处理。不要评论'你已经"
        f"回答过几次了'，不要说'我不会再重复这个问题'，不要因此把学生"
        f"摁回旧题目继续作答。\n"
        f"请立即生成一道全新的题目，类型为【{assigned_type}】"
        f"（判断标准是解题第一步的决策方式，不要只是把旧题换个数字）。"
