...
| 该学生历史证据条数不足以支撑对比判断（例如少于 `PromotionPolicyConfig.window_size` 或某个约定阈值 N） | 样本不足 | **不纳入**本轮对比统计，单独列出"样本不足名单"，不套用上面四条强行判定 |

**关于第五条的背景说明（2026-07-30 明确，实事求是记录，不回避）**：本项目最初瞄准的是中国区用户，但因中国区部署面临较多非技术性困难，目前暂缓投入精力在那个方向，这意味着短期内真实学生规模会持续偏少——这是一个**结构性限制**，不是临时性的数据不足，相当一段时间里需要更多依赖模拟/合成数据（见 FREEZE-02 边缘案例）来补足真实数据的不足。前四条判定标准默认前提是"同一学生有一段有意义长度的连续历史证据"，如果这个前提本身经常不成立，直接套用前四条会产生大量"看似需要排查、实际只是样本太薄没法判断"的噪音，所以需要第五条把这类情况先筛出来，不要混进真正的差异判定里。

**这一条对 Shadow Run 通过标准的影响**：如果"样本不足名单"占比过高（例如超过参与对比学生总数的一半），说明当前阶段做 Shadow Run 的证据强度本身有限，这时"通过"应该被理解为"在现有数据规模下没有发现反对证据"，而不是"已经过充分验证"——这个区别需要在阶段五变更记录里如实注明，不能把数据规模的局限性悄悄掩盖成结论的确定性。
