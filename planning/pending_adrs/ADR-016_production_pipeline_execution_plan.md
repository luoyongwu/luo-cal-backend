...
**执行前置检查**：
- 显式验证两者之间的数据传递契约：`aggregate_dual_scale()` 输出结构是否与 `PromotionPolicy.update()` 期望的输入结构完全一致（防止 numpy float vs Python float、None vs 空 dict 这类沙盒里因"同一作者心里对齐"而未暴露的类型不匹配）。
- **验收项**：在 `PromotionPolicy` 初始化时加入 schema 断言检查，输入不符合预期结构时直接抛出明确错误，不允许错误静默传播到下游。
...
2. 现有 `world_weights` 列是否有 NULL 值或异常类型
3. 同一 `student_id` 是否存在多条记录（不应该有）
4. `aggregator_version` 字段是否存在、是否一致
