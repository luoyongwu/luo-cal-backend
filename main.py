...
#
# 2026-09-07 新增：SV"提议+具体数值"误触发修复
# ---------------------------------------------------------------
# 实证发现（2026-09-07 完整黑盒复测，8组样本，Supabase实测记录，详见
# memory /areas/luo-cal-ole.md "SV retest results" 节）：2026-09-06
# 版定义能正确处理"不带任何方法/数值的纯提议"（如"要不要检查一下？"，
# 复测确认不再误触发），但对"提议句里带了具体数值或方法名称"这一类
# 完全失守——即使句子整体仍是疑问/提议语气、且显式声明"我还没算"，
# 只要句中出现了"代入t=π/2"这类具体操作描述，依然会被判定为满足
# 条件2。同一失败模式复现两次（原始样本+专门追加的"我还没算"反证
# 样本），确认不是随机噪音，而是模型把"句中出现具体数值"错误地当成
# 了"条件2已满足"的判断依据，没有去检查这个具体操作是提议中的还是
# 已执行的。
#
# 修复方式：新增"判断锚点澄清"条款，把判断依据从"句中是否出现具体
# 数值"改为"是否同时存在已完成动作+该动作产生的结果"两者共同成立，
# 并明确说明"我还没算"等未完成声明不能被反过来当作满足条件的依据。
# 同批复测（SV核心边界6/8、SV vs SC、SV vs JR）确认这个问题只影响
# "提议+数值"这一种组合，SC/JR的区隔本身没有发现问题，不需要连带
# 修改。
# ===================================================================
OLE_LABELS = {
    "SPONTANEOUS_VERIFICATION": "核验行为（2026-09-07再修）——学生对一个已经形成的结论/表达实施了一个可观察、可描述的核验操作（如代入检验、换法重算、对称性论证、边界值检验、量纲检查等），而非仅表达核验意图或提议；判断锚点是'已完成动作+该动作的结果'同时存在，而非句中是否出现具体数值（带具体数值的提议句仍不算）；证据允许跨轮追溯；核验发起方（学生主动/教师要求）不影响本标签判定，通过独立的[AGENCY:XXX]标注记录进 teaching_intervention_log.sv_agency 字段，不进入本标签体系",
